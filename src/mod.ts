            ...
            for (const questId in questTable) {
                ...
                switch (quest.type) {
                    case QuestTypeEnum.ELIMINATION:
                        this.logger.info(`Quest name ${quest.QuestName} has ${completionConditions.length} conditions`);
                        completionConditions.forEach(condition => {
                            this.logger.info(`Quest condition type: ${(condition as any).conditionType}`);
                        });
                        break;
                }
            }
